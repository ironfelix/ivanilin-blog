...
export const TRANSLATIONS = {
  ...
  ru: {
    ...
    toc: 'Содержание',
    headingAnchorLabel: (text: string) => `Ссылка на «${text}»`,
    tagPostCount: (count: number) => `${count} постов`,
    allTags: 'Все теги',
    noPosts: 'Пока нет постов.',
    noArchive: 'Пока нет архива.',
    noTags: 'Пока нет тегов.',
    tagsListHeading: 'Теги',
    readAllArchive: 'Весь архив',
    pageFeed: 'Лента',
    feedIntro:
      'Подпишитесь через RSS / Atom читалку, чтобы получать новые посты. Сайт полностью статический, не требует аккаунта или JavaScript.',
    feedRssUrlLabel: 'RSS адрес',
    feedAtomUrlLabel: 'Atom адрес',
    recentPosts: 'Последние посты',
    feedDescription: 'RSS / Atom подписка',
    rssSubscribe: 'RSS лента',
    backToTop: 'Наверх',
    codeBlockLabel: 'Блок кода',
    codeBlockLanguageLabel: (language: string) => `Блок кода, язык ${language}`,
    diffAddedLineLabel: 'Добавленная строка: ',
    diffRemovedLineLabel: 'Удалённая строка: ',
    tableScrollRegionLabel: (caption: string) => `Горизонтально прокручиваемая таблица: ${caption}`,
    tableScrollRegionFallbackLabel: 'Горизонтально прокручиваемая таблица',
    tableCaptionPrefix: 'Таблица: ',
    footnoteRefLabel: (number: string) => `Сноска ${number}`,
    footnoteBackrefLabel: (number: string) => `Вернуться к сноске ${number}`,
    taskCompletedLabel: (task: string) => `Выполненная задача: ${task}`,
    taskIncompleteLabel: (task: string) => `Невыполненная задача: ${task}`,
    prevPost: 'Предыдущий пост',
    nextPost: 'Следующий пост',
    copyrightAdditionalNote:
      'Материалы защищены выбранной лицензией. При копировании сохраняйте ссылку на оригинал.',
    copyrightFormatTemplate: (title: string, author: string, url: string) =>
      `Название: ${title}\\nАвтор: ${author}\\nОригинал: <a href="${url}">${url}</a>`,
    copyrightLicenseNotices: COPYRIGHT_LICENSE_NOTICES.ru,
  },
} as const;
